...
def save_weights(agent, env, weights_directory, trial):
    ...
    try:
        agent.actor_model.save_weights(
            weights_directory + "actor-trial" + str(trial) + "_" + save_name + ".h5"
        )
    except:
        print("actor save fail")
    try:
        agent.critic_model.save_weights(
            weights_directory + "critic-trial" + str(trial) + "_" + save_name + ".h5"
        )
    except:
        print("critic save fail")


def random_action(agents, disc_actions_num):
    a = np.random.uniform(0, 1, (agents, disc_actions_num))
    b = np.sum(a, axis=1)
    c = np.vstack((b, b) * disc_actions_num)[:disc_actions_num].T
    d = a / c
    return d
